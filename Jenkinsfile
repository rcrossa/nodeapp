  ...
  stages {
    stage('Cloning') {
      parallel {
        ...
        stage('Notificacion') {
          steps {
            slackSend(channel: '#gitHub-update', color: '#439FE0', message: 'Incio de proceso ${env.BRANCH_NAME}', teamDomain: 'devtesis', tokenCredentialId: 'jenkins-devops-projects', iconEmoji: ':ojos')
          }
        }
